...
clf = pickle.load(open('bike_model_xgboost.pkl', 'rb'))
...
def predict():
    ...
    output = round(prediction[0], 2) #rounding the output to 2 decimal places

    return render_template('index.html', prediction_text = 'Number of bikes is :{}'.format(output))
